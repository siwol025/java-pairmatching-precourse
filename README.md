...
- 기능 선택
    - 프로그램을 시작하면 기능의 종류를 출력하고 사용자로부터 그 중 하나의 입력을 받는다.
        - “1” 을 입력 시 페어 매칭
        - “2” 을 입력 시 페어 조회
        - “3” 을 입력 시 페어 초기화
        - “Q” 을 입력 시 프로그램 종료
        - 위의 값 이외의 값을 입력시 에러 처리
            - `IllegalArgumentException`를 발생시키고, `[ERROR]`로 시작하는 에러 메시지를 출력 후 해당 부분부터 다시 입력을 받기    


- 파일로부터 각 과정별 크루원 이름 목록 읽어오기


- 페어 매칭 기능
    - 과정와 미션을 출력하고 매칭하고자 하는 과정, 레벨, 미션을 입력 받기
    - 페어 매칭 조건에 맞게 페어 매칭하기
        - 미션을 함께 수행할 페어를 같은 과정의 크루 두명씩 랜덤 매칭
        - 페어 매칭 대상이 홀수인 경우 한 페어는 3인으로 구성
        - 같은 레벨에서 이미 페어를 맺은 크루와는 다시 페어로 매칭될 수 없다
            - 이 경우 안내 문구 출력 후  다시 매칭 진행
            - 아니오를 선택할 경우 코스, 레벨, 미션을 다시 선택
            - 3회 시도까지 매칭이 되지 않거나 매칭을 할 수 있는 경우의 수가 없으면 에러 처리
                - `IllegalArgumentException`를 발생시키고, `[ERROR]`로 시작하는 에러 메시지를 출력 후 해당 부분부터 다시 입력을 받기      


- 페어 조회 기능
    - 과정, 레벨, 미션을 선택하면 해당 미션의 페어 정보를 출력
    - 매칭 이력이 없으면 매칭 이력이 없다는 에러 메세지출력
        - `IllegalArgumentException`를 발생시키고, `[ERROR]`로 시작하는 에러 메시지를 출력 후 해당 부분부터 다시 입력을 받기  


- 페어 초기화 기능
    - 모든 과정, 레벨, 미션의 페어 매칭 결과를 초기화
    - 안내 문구 출력
